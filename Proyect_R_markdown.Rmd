...
```{r}
load_stroke_data <- function() {
  # Set the file path
  path <- "/Users/soroush/Desktop/UC3M/courses/R/minitask1/Stroke_data/healthcare-dataset-stroke-data.csv"
  
  # Read the CSV file
  stroke_data <- read.csv(path, header = TRUE)
  
  # Remove 'Other' gender and convert columns to factors
  stroke_data <- stroke_data[stroke_data$gender != "Other", ]
  factor_columns <- c("gender", "hypertension", "heart_disease", "work_type", 
                      "Residence_type", "ever_married", "smoking_status", "stroke")
  
  for (col in factor_columns) {
    stroke_data[[col]] <- as.factor(stroke_data[[col]])
  }
...
```{r}
ggplot(stroke_data)+aes(x=stroke,fill=heart_disease) + geom_bar(position=position_dodge())+theme_minimal()
...
```{r}
ggplot(stroke_data)+aes(x=stroke,fill=ever_married) + geom_bar(position=position_dodge())+theme_minimal()
...
```{r}
ggplot(stroke_data)+aes(x=stroke,fill=Residence_type) + geom_bar(position=position_dodge())+theme_minimal()
...
```{r}
ggplot(stroke_data)+aes(x=stroke,fill=smoking_status) + geom_bar(position=position_dodge())+theme_minimal()
...
ggplot(stroke_data, aes(x = interaction(stroke, gender), y = avg_glucose_level, fill = interaction(stroke, gender))) +
...
stroke_data$hypertension <- factor(stroke_data$hypertension, 
                            levels = c(0, 1), 
                            labels = c("No Hypertension", "Hypertension"))
...
stroke_data$ever_married <- factor(stroke_data$ever_married, levels = c("No", "Yes"))
...
stroke_data$stroke <- factor(stroke_data$stroke, levels = c(0, 1), labels = c("No Stroke", "Stroke"))
stroke_data$ever_married <- factor(stroke_data$ever_married, levels = c("No", "Yes"), labels = c("Never Married", "Ever Married"))
...
age_freq_stats <- stat.freq(stroke_data$age)
...
stroke_data$heart_disease <- factor(stroke_data$heart_disease, 
                             levels = c(0, 1), 
...
ggplot(stroke_data, aes(x = smoking_status, y = age, fill = stroke)) +
